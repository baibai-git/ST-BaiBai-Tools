...
- 柏宝库分页/加速：Settings、扩展 Manifest 合并请求、角色列表、最近聊天、美化主题懒加载、生成结果后端自动保存、Tokenizer 批量计数
...
- `生成结果后端自动保存`：普通单角色 Chat Completion 生成改用柏宝库 save-generate，生成完成后由后端写入聊天文件。
...
启用 `生成结果后端自动保存` 后，普通单角色 Chat Completion 的 `normal` 和 `regenerate` 回复会走柏宝库 save-generate 接口。前端仍按 SillyTavern 原流程发起生成，但后端会在生成完成后直接把 AI 回复写入当前聊天文件，避免手机浏览器切后台、刷新或断线导致“前端拿到回复但还没来得及保存”的丢失场景。
