...
黑名单每行一条，按正文包含文本匹配，忽略英文大小写和空行，不支持正则表达式。单聊普通生成和重新生成完成并收尾后，读取本轮最新助手楼层的正文，命中任一条就调用 ST 原生重新生成，替换命中回复，不增加候选回复或重复发送用户消息。默认每轮合计最多额外重试 3 次，可设为 1 到 10 次；黑名单重试间隔 1.5 秒，等待期间可用酒馆停止按钮取消。总次数耗尽仍命中时，停止重试、保留最后回复并提示，由用户决定后续处理。

流式和非流式均支持；流式请求异常结束后，已显示的错误正文也会检查黑名单（例如 `HTTP 503 Service Unavailable`），不会把错误中止当成用户主动停止。不扫描历史聊天或独立思维链字段，不处理续写、候选回复、群聊和其他插件独立发起的辅助生成。开启 ST 原生自动切换回复时，本功能暂停以避免重复触发。手动停止、切换聊天、编辑或切换当前生成楼层的候选回复、删除消息或修改黑名单设置会取消当前重试链；更新无关旧楼层不会取消。重试等待期间若聊天再次保存，会继续等保存收尾，不会直接丢掉这次重试。
